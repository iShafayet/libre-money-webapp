...
export const dataInferenceService = {
  async updateCurrencyCache() {
    currencyCacheList = (
      await pouchdbService.listByCollection(Collection.CURRENCY)
    ).docs as Currency[];
  },

  prettifyAmount(amount: number, currencyId: string) {
    const currency = currencyCacheList.find(
      (_currency) => _currency._id === currencyId
    );
  ...
  async inferRecord(record: Record): Promise<InferredRecord> {
    ...
    if (inferredRecord.type === RecordType.EXPENSE && inferredRecord.expense) {
      inferredRecord.expense.expenseAvenue = await this.getExpenseAvenue(
        inferredRecord.expense.expenseAvenueId
      );

      if (inferredRecord.expense.partyId) {
        inferredRecord.expense.party = await this.getParty(
          inferredRecord.expense.partyId
        );
      }
    }
  ...
  async getExpenseAvenue(expenseAvenueId: string) {
    const doc = (await pouchdbService.getDocById(
      expenseAvenueId
    )) as ExpenseAvenue;
    return doc;
  },
  ...
  async getTagList(tagIdList: string[]) {
    const docList = (await pouchdbService.listByCollection(Collection.TAG))
      .docs as Tag[];
