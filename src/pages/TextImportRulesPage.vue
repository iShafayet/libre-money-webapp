...
export default defineComponent({
  ...
  setup() {
    ...
    async function addRuleClicked() {
      $q.dialog({ component: AddTextImportRule }).onOk((res) => {
        loadData();
      });
    }
    ...
    async function editClicked(rule: TextImportRules) {
      ...
      $q.dialog({ component: AddTextImportRule, componentProps: { existingRuleId: rule._id } }).onOk((res) => {
        loadData();
      });
    }
